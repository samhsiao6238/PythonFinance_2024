...
2. 查看當前用戶的權限，結果表明用戶能夠登錄 MariaDB，但無法對數據庫執行任何操作，除非有其他授權。。
...
3. 直接在當前用戶指令下切換到管理員；其中 `\!` 表示執行的是系統指令，接著要輸入的是 root 的密碼，也就是 NAS 密碼。
...
6. 授予 `CREATE USER` 權限；特別強調，前面的 `ALL PRIVILEGES` 並不包含建立使用者的權限。
...
7. 還要授予 `GRANT OPTION` 權限給 sam6238。
...
9. 查看當前用戶是否擁有足夠的權限；以下將顯示當前用戶 `sam6238@%` 的所有權限。
