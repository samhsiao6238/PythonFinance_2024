...
_以下諸多單元將針對購物網 [酷澎](https://www.tw.coupang.com/) 進行商品價格查詢，進一步拓展寫入資料庫，並透過網頁伺服器查詢資料庫數據；最終將應用部署在樹莓派，並透過外網進行訪問。_

<br>

## 準備工作

_避免 Chrome 與 chromedriver 版本不匹配，運行以下步驟清除快取，並查詢兩者版本；特別注意，ChromeDriver 通常允許 patch-level 的差異。_

<br>

1. 清除 webdriver-manager 快取的舊 driver。
...
_以 `商品關鍵字` 在網站上查詢數據資料，接著在取回的數據中進行 `進階篩選`_
