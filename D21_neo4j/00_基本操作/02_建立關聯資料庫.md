...
6. 若要避免資料重複，則需使用 `MERGE` 指令替代 `CREATE`，因為 `MERGE` 會檢查節點是否已存在，如果存在就會使用現有的節點，如果不存在則會創建一個新節點，這可確保數據庫中不會因為重複執行而產生重複的節點。
    ...
    def create_complex_relationships():
        """在資料庫中插入複雜的電影與演員節點及其關係，確保節點不重複創建"""
        """使用 MERGE 確保電影節點不會重複創建"""
        cypher_query = """
        // 電影
...
1. 延續上點，若資料中具有雙向關係可進行雙向查詢，可在語法中添加以下代碼，特別注意，`Neo4j` 中的關係是有方向性的，若要建立雙向關係則需要建立兩次。
