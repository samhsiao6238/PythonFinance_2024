...
1. 這個筆記是基於官方的 [GraphAcademy 教程](https://graphacademy.neo4j.com/courses/llm-chatbot-python/1-project-setup/) 所進行的拓展，不過教程略在後半段不太清楚且有部分錯誤，這裡做個修正與彙整。

    ![](images/img_01.png)

<br>

2. 範例中的腳本可參考 [GitHub](https://github.com/neo4j-graphacademy/llm-chatbot-python?tab=readme-ov-file)，在操作中也會透過 `Git` 指令克隆這個倉庫，並且在這個筆記中對其做使用與修正。

<br>

3. 關於 `向量索引`的部分，可以參考 [LangChain 官方文件 - Neo4j Vector Index](https://python.langchain.com/v0.1/docs/integrations/vectorstores/neo4jvector/)，或參考 [neo4j 官方文件 － Vector search indexes](https://neo4j.com/docs/cypher-manual/current/indexes/semantic-indexes/vector-indexes/)。
...
1. 安裝套件；透過指令安裝 `requirements.txt` 文件中指定的模組與對應的版本；另外，指令中透過 `echo` 添加一個套件，這裡是因為實作範例後發現缺少了這個套件，故在此將其加入。
...
10. 補充前面提及的 `langchainhub` 模組，因為在後續的運作中會報錯缺少這個套件，通常可在當下再透過以下指令手動進行安裝，而這裡先行將它加入 `requirements.txt` 批次安裝，如此可避免後續建立容器時需要在 `devcontainer.json` 文件中添加指令的設置。
