...
2. 將本地的 Git 儲存庫與 Hugging Face 平台上的遠端儲存庫進行關聯，這樣就可以將本地的代碼和文件推送到 Hugging Face 的遠端儲存庫中。
