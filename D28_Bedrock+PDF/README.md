...
2. Base64 編碼：將 PDF 文件內容編碼為 Base64，因為 Base64 編碼可以將二進制數據轉換為 ASCII 字符串，便於在 HTTP 請求中傳遞文件內容。
...
3. 使用 AWS Bedrock Model ARN：根據 AWS 區域和模型 ID 構建模型 ARN，傳遞給 `retrieve_and_generate` 方法。
