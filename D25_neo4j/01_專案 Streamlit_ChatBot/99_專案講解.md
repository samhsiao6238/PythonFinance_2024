...
4. 關於 `向量索引`的部分，可以參考另外筆記，也可參考 [LangChain 官方文件 - Neo4j Vector Index](https://python.langchain.com/v0.1/docs/integrations/vectorstores/neo4jvector/)，或 [neo4j 官方文件 － Vector search indexes](https://neo4j.com/docs/cypher-manual/current/indexes/semantic-indexes/vector-indexes/)。
...
2. 建立虛擬環境：開啟終端機，透過指令進入自己管理 Python 虛擬環境的路徑中建立，這裡示範的路徑是 `~/Documents/PythonVenv`。
    ...
    _`MacOS` 若要指定 Python 版本_

    ```bash
    /Library/Frameworks/Python.framework/Versions/3.11/bin/python3.11 -m venv envPython311
    ```
