...
5. tempfile：用於創建臨時文件和目錄，提供了一個安全的方式來創建臨時文件，這些文件在使用後可以自動刪除。

6. time：提供時間相關的功能，支持獲取當前時間、進行時間計算、模擬延遲等操作。
...
1. vectorstores.pgvector：使用 pgvector 進行向量存儲，支持高效的向量檢索操作。
...
5. llms.bedrock.Bedrock：用於與 Bedrock LLM 進行交互，支持不同的 LLM 模型。

6. embeddings.BedrockEmbeddings：用於生成文本的嵌入表示，以便在向量存儲中使用。

7. document_loaders.S3FileLoader：用於從 S3 中加載文件。

8. document_loaders.csv_loader.CSVLoader：用於加載和處理 CSV 文件。

9. document_loaders.YoutubeLoader：用於從 YouTube 加載和處理字幕或轉錄文本。

10. document_loaders.UnstructuredPowerPointLoader：用於處理和加載 PowerPoint 文件。

11. document_loaders.Docx2txtLoader：用於處理和加載 Word 文件。

12. memory.PostgresChatMessageHistory：用於管理和存儲對話歷史的 PostgreSQL 記錄。

<br>

## 步驟

1. 因為使用了多種套件如前述，基於版本依賴與衝突考量，建議建立新的虛擬環境來運行專案，這裡我測試的是環境名稱是 `env0604`。
...
3. 可查看 `requirements.txt` 文件中各套件的指定版本。
...
6. _特別注意_，因為我調用 `os.getenv()` 在腳本最開始處取得環境參數，所以也要在腳本最開始處調用 `load_dotenv()`；若採用原始範例的方式使用 `os.environ.get()`，則可以在 `if __name__ == '__main__'` 區塊再調用 `load_dotenv()` 來載入環境參數。
...
10. 經查當前腳本預設使用的是 `v1`，尚未取得 `AWS` 的許可。
...
11. 進入 AWS 申請 `v2`，這個模型會立即取得授權，無需等待。
...
16. 這是因為提供的 PDF 文本中存在 `NUL 字串（0x00）`，這會導致系統無法正常處理這些數據，因為使用 `PGVector.from_texts` 或 `PGVector` 時，這種字串會導致無法存儲或處理文本數據，需要在讀取和處理 PDF 文件時過濾掉這些 NUL 字串；以下修改 `get_pdf_text` 以及 `get_text_chunks` 函數以加入過濾 NUL 字串的邏輯。
...
17. 再次提交 `PDF` 文件後正確讀取，在側邊欄可輸入要提問的問題。
