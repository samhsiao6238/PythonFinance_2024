...
1. `Gradio` 是一個用於建立和分享機器學習應用的 Python 庫，它使開發者能夠輕鬆地將機器學習模型轉化為具有圖形用戶界面的應用程序，並且可以在幾行程式碼內完成這一過程。
...
1. Gradio 的 API 非常簡單，幾乎不需要額外的設置，只需要幾行程式碼就可以建立一個互動式界面。

<br>

2. 支持多種 _輸入格式_，如文本、圖像、音頻、影片等，_輸出_ 可以是文本、圖像等。
