...
1. 前面提過的 `genai.ChatSession` 提供一個方便的封裝，也就是封裝了 `GenerativeModel.generate_content` 的更高層級接口，可用於處理多輪對話；而使用 `GenerativeModel.generate_content` 管理多輪對話可提供更大的控制，適合需要自定義對話歷史管理或特殊處理的情境。

2. `Gemini` 支援 `多輪對話`，可使用 `ChatSession` 管理 `對話狀態`，以下代碼展示如何初始化一個聊天會話。
...
2. 透過新增回覆內容和其他訊息來 `延續對話`，並指定使用流式輸出 `stream=True` 來串流聊天內容。
...
3. 繼續對話，同樣使用 `chat.send_message` 來發送訊息。
