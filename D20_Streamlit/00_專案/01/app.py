...
# 頁面設置，特別注意，這裡要使用「#」，不可使用「''''''」
# 因為「''''''」在 Streamlit 中視為腳本內容，只有「#」代表的是註解
# 所有在腳本中以「''''''」標註的內容都將呈現在網頁中
# 而「set_page_config」必須是最開始的腳本，所以在此之前不可有其他程式碼
st.set_page_config(
...
def calculate_population_difference(input_df, input_year):
    ...
    previous_year_data = input_df[
        input_df["year"] == input_year - 1
    ].reset_index()
    (
        selected_year_data["population_difference"]
    ) = selected_year_data.population.sub(
...
with col[0]:
    ...
    if selected_year > 2010:
        ...
        last_state_delta = ""
    st.metric(
        label=last_state_name,
        value=last_state_population,
    ...
    if selected_year > 2010:
        ...
        df_greater_50000 = df_population_difference_sorted[
            (
                df_population_difference_sorted.population_difference_absolute
            ) > 50000
        ]
        '''
        ...
        states_migration_greater = round(
            (len(df_greater_50000) / (
                df_population_difference_sorted.states.nunique()
            ))
            * 100
        )
...
with col[2]:
    ...
    with st.expander("About", expanded=True):
        st.write(
            ...
            - :orange[**Gains/Losses**]: states with high inbound/ \
                outbound migration for selected year
            - :orange[**States Migration**]: percentage of \
                states with annual inbound/ outbound migration > 50,000
            """
        )
