...
8. 影片幀處理循環。
   ...
   while videoCapture.isOpened():
      ...
      if success:
         # 使用 `process_image` 函數來處理每一幀，並根據設定顯示或保存結果。
         process_image(img)
         # ...
   ```
...
# 設定常數
SAVE_RESULT_AS_VIDEO = False  # 如果為 True，則保存結果為影片
RESULT_FILE_NAME = "result.mp4"  # 輸出影片的檔案名稱
YOLO_MODEL_PATH = "yolov8n.pt"  # 使用的 YOLO 模型文件
