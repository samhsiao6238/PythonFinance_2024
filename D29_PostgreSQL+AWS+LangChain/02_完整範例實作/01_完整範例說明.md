...
1. `os.getenv()` 在調用時會直接查找環境變數，因此必須在腳本一開始調用 `load_dotenv()` 來載入環境變數，否則 `os.getenv()` 將無法獲取正確的值。

<br>

2. `os.environ.get()` 查找的是當前環境中的變數，可以在任何時候查找，只要在查找之前環境變數已經被載入到 os.environ 中。因此，可以在腳本運行過程中的任何地方載入環境變數。
