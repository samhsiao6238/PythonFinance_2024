...
16. 這是因為提供的 PDF 文本中存在 `NUL 字串（0x00）`，這會導致系統無法正常處理這些數據，因為使用 `PGVector.from_texts` 或 `PGVector` 時，這種字串會導致無法儲存或處理文本數據，需要在讀取和處理 PDF 文件時過濾掉這些 NUL 字串；以下修改 `get_pdf_text` 以及 `get_text_chunks` 函數以加入過濾 NUL 字串的邏輯。
