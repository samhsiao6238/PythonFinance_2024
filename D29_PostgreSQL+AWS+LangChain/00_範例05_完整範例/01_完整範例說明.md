...
5. _特別注意_，因為我調用 `os.getenv()` 在腳本最開始處取得環境參數，所以也要在腳本最開始處調用 `load_dotenv()`；若採用原始範例的方式使用 `os.environ.get()`，則可以在 `if __name__ == '__main__'` 區塊再調用 `load_dotenv()` 來載入環境參數。
