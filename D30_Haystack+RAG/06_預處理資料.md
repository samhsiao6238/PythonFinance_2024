...
7. 接下來，設定 `預處理管道`的 `connect` 方法，將不同類型的文件進行轉換、清理、嵌入後寫入文件存儲中。
...
2. 用食譜文件 `recipe.txt` 來測試這個管道：說明一下這個食譜文件下載的時候全名是 `vegan_sunflower_hemp_cheese_recipe.txt`，我手動進行修改為短名稱，目的是在閱讀輸出的時候比較簡潔。
...
3. 構建一個 `RAG 管道` 來根據剛剛創建的文件回答查詢；在這一步驟中將會使用到 `HuggingFaceAPIGenerator`，因此需要擁有一個 `Hugging Face API Key`，同時將使用 `HuggingFaceH4/zephyr-7b-beta` 模型。
