...
4. 這個範例的目標就是在索引文件時 `嵌入元數據信息`，這可改進檢索效果，對於哪種類型的元數據適用當前所以需視狀況，此例中演示嵌入 `title` 元數據欄位為嵌入模型提供良好的上下文，實作上將從維基百科獲取不同頁面，並將其與元數據（包括標題和 URL）一起索引到 `InMemoryDocumentStore` 中。
