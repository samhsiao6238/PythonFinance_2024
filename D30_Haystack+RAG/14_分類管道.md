...
4. 本範例最終目標是構建一個 `Haystack 管道` 來根據 `文件的語言` 進行分類，還可以將 `語言分類` 和 `查詢路由` 集成到 `RAG 管道`中，如此便能根據 `查詢的語言` 進行文件檢索，並使用相應語言的文件來輔助生成答案。
...
8. `PromptBuilder`：提示生成器，用於構建提示文本，將用戶的查詢和文件內容結合成一個統一的提示，供生成模型使用。
...
2. 導入 `RAG 管道` 所需的組件。
    ...
    # 用來根據模板構建提示，將查詢和相關文件整理成一個統一的提示文本
...
1. 為了構建 `多語言 RAG 管道`，將使用 `TextLanguageRouter` 來檢測查詢的語言，然後從相應的文件儲存中獲取該語言的文件。另外，假設之前放入 `文件儲存` 中的評論都是針對同一個住宿的，使用 `RAG 管道`，可以根據選擇的語言查詢有關該公寓的信息。
    ...
    # PromptBuilder 用於根據模板構建提示文本，將查詢和相關文件整理成一個提示文本
    rag_pipeline.add_component(
        # 指定提示模板
