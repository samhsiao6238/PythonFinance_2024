...
4. `DocumentWriter`：文件寫入器，用於將文件寫入到指定的文件儲存中。這個組件可以將處理後的文件（例如經過嵌入向量化的文件）保存到內存、資料庫或其他持久化儲存中，以便後續檢索和查詢。
...
1. 為了建立 `多語言 RAG 管道`，將使用 `TextLanguageRouter` 來檢測查詢的語言，然後從相應的文件儲存中取得該語言的文件。另外，假設之前放入 `文件儲存` 中的評論都是針對同一個住宿的，使用 `RAG 管道`，可以根據選擇的語言查詢有關該公寓的信息。
