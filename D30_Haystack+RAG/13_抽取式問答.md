...
1. 本範例使用 `自然語言處理技術` 來理解問題的語義，然後結合 `向量檢索技術` 從大量文本中查找並提取問題的 `最相關的文字片段`，由於不是生成新的文本來回答，可確保答案的準確性和可驗證性。同時還可標示出答案在原文本中的位置，使得用戶可以查閱或核實答案的來源。
...
2. `InMemoryDocumentStore`：是一個內存中的文檔存儲，它用來存儲和管理文檔數據，方便後續的檢索和查詢操作。

<br>

3. `InMemoryEmbeddingRetriever`：一個用來檢索相關文檔的組件，它會根據查詢和文檔的嵌入向量來查找最相關的文檔。

<br>

4. `DocumentWriter`：是一個負責將文檔寫入到文檔存儲的組件，通常用於將處理後的文檔存儲到內存或數據庫中。

<br>

5. `SentenceTransformersDocumentEmbedder`：是用來將文檔轉換為嵌入向量的組件，這些向量可以用於後續的檢索和查詢。

<br>

6. `SentenceTransformersTextEmbedder`：是用來將查詢文本轉換為嵌入向量的組件，它通常用於將查詢轉換為與文檔同樣的向量表示。
...
2. 文本嵌入：將清理好的文本通過嵌入模型轉換成嵌入向量，這些嵌入向量將用於計算文本之間的相似度，並進行相關的檢索操作；範例中使用了 `SentenceTransformersDocumentEmbedder` 組件將文檔轉換為嵌入向量。

<br>

3. 檢索：使用 `InMemoryEmbeddingRetriever` 組件進行檢索，這個組件會根據查詢的嵌入向量從文檔存儲中檢索出最相關的文檔。檢索器負責找查相關的文檔，讀取器則負責從文檔中抽取具體的答案，並返回答案的位置和信心分數。

<br>

4. 問答：使用 `ExtractiveReader` 組件進行問答。這個組件會從檢索到的文檔中抽取出具體的答案，並提供答案的置信度分數。

<br>

5. 文檔管理：使用 `DocumentWriter` 將向量化的文檔寫入 `DocumentStore`，以確保這些文檔可以在需要時快速檢索和使用。這裡使用的文件儲存事內存文檔存儲（`InMemoryDocumentStore`）。
...
3. 加載與儲存資料，數據加載後會存儲在內存中，也就是本地的 `.cache` 目錄。
...
2. 進行 `向量搜索` 時，檢索器會從文件存儲中高效地返回相關文件，所以檢索器與文件存儲是緊密耦合的，這裡使用檢索器 `InMemoryEmbeddingRetriever` 來搭配 `InMemoryDocumentStore`。
