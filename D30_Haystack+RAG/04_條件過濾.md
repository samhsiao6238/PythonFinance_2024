...
1. 在 Haystack 中，`score` 表示 `文檔` 與 `查詢` 之間的 `相似度分數`，這個分數由檢索或排序算法計算，在這個範例中使用的是 `BM25 檢索方法`，並指定在管道之中，其表示數值在 `0 到 1` 之間，值越高表示匹配程度越高。

<br>

2. `score` 會用於 `排序` 檢索結果，高分數的文檔通常會排在搜索結果的前面，補充說明一下，`BM25 算法` 會基於查詢詞在文檔中的出現次數和文檔長度來計算相似度，而在向量檢索方法時可能會基於文檔和查詢向量之間的 `餘弦相似度` 來計算。
