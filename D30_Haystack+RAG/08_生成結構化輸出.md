...
1. 參考 `2024/04/25` 的 [官方教程](https://haystack.deepset.ai/tutorials/28_structured_output_with_loop)，目標是建立一個系統來使用 `Haystack 2.0` 的 `循環管道` 和 `LLM` 進行動態的數據處理，透過使用了 `LLM` 從 `非結構化數據` 中提取 `結構化數據`，並驗證生成的輸出是否符合預定義的結構。

<br>

2. 使用 GPT 模型將非結構化段落轉換為符合 `Pydantic` 結構的 JSON 輸出，並使用自定義的 `OutputValidator` 組件來驗證 JSON 並在必要時進行校正。
