...
## 創建自定義組件 OutputValidator

1. `OutputValidator` 是一個自定義組件，用於驗證 `LLM` 生成的 `JSON` 對象是否符合提供的 `Pydantic` 模型，如果不符合，`OutputValidator` 會返回錯誤消息以及不正確的 JSON 對象，並在下一個循環中進行修正。可參考官方對於 [創建自定義組件](https://docs.haystack.deepset.ai/docs/custom-components) 的說明。
...
2. 接著，使用之前創建的 `CitiesData` 創建一個 `OutputValidator` 實例。
...
1. 為 `LLM` 編寫將段落轉換為 `JSON` 格式的指令。確保指令說明如何識別和修正錯誤，如果 JSON 不符合要求的結構。一旦創建了提示，初始化 `PromptBuilder` 以使用它。關於 Jinja2 模板和 `PromptBuilder` 可參考 [官方 PromptBuilder 說明](https://docs.haystack.deepset.ai/docs/promptbuilder)。
