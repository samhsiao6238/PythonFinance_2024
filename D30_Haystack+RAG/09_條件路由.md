...
1. 這是在 `2024年4月25日` 發佈的 [官方教程](https://haystack.deepset.ai/tutorials/36_building_fallbacks_with_conditional_routing)。

<br>

2. 使用 `Haystack` 建立具有條件路由的管道，當查詢在數據集中找不到答案時，會自動退回到網絡搜索進行搜尋。

<br>

3. 使用的組件有 `ConditionalRouter`、`SerperDevWebSearch`、`PromptBuilder`、`OpenAIGenerator`，除此也會使用到 `OpenAI API` 和 `Serper API`。

<br>

4. 在使用 `檢索增強生成（RAG）` 進行應用開發時，`檢索步驟` 是 `LLMs` 生成 `回應` 的主要信息來源，當數據庫缺乏所需信息，可使用網絡作為 `RAG 應用` 的數據來源是一種實用的解決方案，這可通過在系統中實現 `條件路由機制` 控制數據流，設計在某些情況下利用網絡作為數據源的系統。

<br>

5. 簡言之，這個範例的目標就是建立一個具有 `條件路由`的 `管道`，當在數據集中找不到答案時，可以自動回退到網絡搜索。

<br>

## 關於 serper

1. [官網](https://serper.dev/) 進行登入與註冊。
...
1. 定義一個提示作為指導 LLM 的回答，如果提供的文件中沒有足夠的上下文來回答查詢，則回覆 `no_answer`。接著，用這個提示初始化一個 `PromptBuilder`；LLM 回覆`no_answer` 這個關鍵詞會指示系統應該將查詢導向回退的網絡搜索路徑。
...
1. 初始化網絡搜索 RAG 應用所需的組件，除了 `PromptBuilder` 和 `OpenAIGenerator`，還需要一個 `SerperDevWebSearch` 來從網絡上檢索與查詢相關的文件。另外，也可以為基於網絡的 RAG 分支使用不同的生成器。
...
2. 定義兩個路由，如果 LLM 回覆 `no_answer` 關鍵詞，管道應該進行網絡搜索，也就是將把原始查詢作為輸出值傳遞給下一個 `組件`，在這種情況下，下一個組件將是 `SerperDevWebSearch`，並且 `輸出名稱` 將是 `go_to_websearch`。除此，提供的文件已足夠回答問題，管道執行在此結束，並將 `LLM` 回覆的答案作為名為 `answer` 的輸出。
...
1. 將所有 `組件` 添加到 `管道` 中並進行 `連接`，`go_to_websearch` 路由的輸出應該連接到 `websearch` 以從網絡檢索文件，同時也應該連接到 `prompt_builder_for_websearch` 用於生成提示。
...
1. 若要觀察何使用條件路由構建這個管道，可使用 `draw()` 方法生成管道圖。
...
1. 在 `run()` 中，將查詢傳遞給 `prompt_builder` 和 `router`，特別注意，在實際應用中，文件通常由 `檢索器` 提供，這裡基於簡化示範，將自行定義的文件直接提供給 `prompt_builder`。
