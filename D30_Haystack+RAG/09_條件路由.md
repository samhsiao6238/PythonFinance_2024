...
2. 使用 `Haystack` 建立具有 `條件路由` 的 `管道`，當在 `數據集` 中找不到答案時，會自動退回到 `網絡` 進行搜尋。

<br>

3. 使用的組件有 `ConditionalRouter`、`SerperDevWebSearch`、`PromptBuilder`、`OpenAIGenerator`，除此也會使用到 `OpenAI API` 和 `Serper API` 兩個服務。

<br>

4. 在使用 `檢索增強生成（RAG）` 進行應用開發時，`檢索步驟` 是 `LLMs` 生成 `回應` 的主要信息來源，當數據庫缺乏所需信息，改用網絡作為 `RAG 應用` 的數據來源是一種實用的解決方案；這可通過在系統中實現 `條件路由機制` 控制數據流，設計在某些情況下利用網絡作為數據源的系統。

<br>

5. 簡言之，這個範例的目標就是建立一個具有 `條件路由`的 `管道`，當在數據集中找不到答案時，可以自動改用網絡進行搜索。
...
2. 註冊完成點擊 `API Key` 來生成密鑰，並將密鑰寫入 `.env`，鍵值使用 `SERPERDEV_API_KEY`。
...
1. 定義一個 `提示` 來指導 LLM 的回答，明確指示在提供的文件中沒有足夠的上下文來回答時回覆 `no_answer`，接著用這個 `提示` 初始化 `PromptBuilder`，當 LLM 回覆`no_answer` 這個關鍵詞時，系統會依據指示退回到的網絡進行搜索。
...
1. 若要觀察何使用條件路由建立這個管道，可使用 `draw()` 方法生成管道圖。
