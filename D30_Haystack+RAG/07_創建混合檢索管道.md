...
2. 初始化 DocumentStore：通過初始化 `DocumentStore` 來開始創建問答系統。`DocumentStore` 用於儲存系統用於查找問題答案的文件。在本範例中將使用 `InMemoryDocumentStore`。
...
3. 在 `Hugging Face Hub` 上有許多來自 PubMed 的數據集，所以範例將使用 `anakin87/medrag-pubmed-chunk`，透過 `for-in` 從數據集中創建文件，在 `PubMed` 的數據集中，每個數據點都有 4 個特徵。
...
_創建一個管道，將數據儲存在 `文件儲存 document store` 中並生成嵌入_
...
2. 使用 `SentenceTransformersDocumentEmbedder` 來創建用於密集檢索的文件嵌入；作為嵌入模型，將使用 `Hugging Face` 上的 `BAAI/bge-small-en-v1.5`。可嘗試在 `Hugging Face` 上測試其他 `模型` 或使用其他 `嵌入器` 來切換模型提供商；如果太過耗時，可將替換較小的嵌入模型，如 `sentence-transformers/all-MiniLM-L6-v2` 或 `sentence-transformers/all-mpnet-base-v2`。請確保根據模型的 token 限制更新 `split_length`。
...
6. 程式碼。
    ...
    _至此文件已儲存在 `InMemoryDocumentStore` 中並生成了嵌入，接下來可創建混合檢索管道_
...
4. 創建混合檢索管道：將所有初始化的元件添加到的管道中並進行連接。
