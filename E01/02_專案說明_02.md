...
1. 關於 `np.random.normal()`：這是 Numpy 中用於生成 `常態分配隨機數` 的函數，參數 `loc=0.0002` 用於指定平均值，這是用於表示投資組合每日平均報酬率，參數 `scale=0.001` 用於指定常態分配的標準差，在統計中，標準差代表的是數據的分散程度，也就反映了波動與風險程度；參數 `size=len(dates)` 用於指定生成隨機數的數量，結果要與 `dates` 適配，確保每日都有一個隨機的報酬率；`.cumsum()` 是 Array 的函數，用於計算給定軸向上的累積和，也就是 `np.random.normal()` 所生成隨機數的和。
