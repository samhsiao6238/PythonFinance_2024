...
1. 載入了預訓練的模型和 tokenizer： 在這段代碼運行之前，應該已經使用了例如 Hugging Face 的 transformers 庫來載入所需的模型和 tokenizer。
...
2. 已經定義了 input_ids： 在代碼運行之前，應該已經有了 input_ids，這是輸入文本通過 tokenizer 編碼後得到的張量。
...
3. 完成以上步驟，在 Mac 系統上使用 MPS（Metal Performance Shaders）。
