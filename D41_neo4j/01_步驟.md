...
5. 目前資料庫中並無資料，所以先建立一個 `simulate_data.ipynb` 的腳本來生成模擬的數據，同時將數據寫入資料庫，腳本名稱自訂即可。
...
8. 在前面的步驟已經建立了 `utility.py`，並封裝了建立連線任務 `get_neo4j_session` 與載入數據 `load_movie_data` 兩個函數，這裡改寫 `app.py` 載入模組 `utility.py`。
...
10. 會自動開啟瀏覽器。
    ...
    _特別注意，這時瀏覽器會出現錯誤提示，但網頁是正常的，這部分在下一個步驟處理 。_
...
3. 為了有效達成這個目的，可搭配 Python 的 `上下文管理器 with` 語句來自動調用內建的資源初始化與清理方法，一但使用了 `with`，會在進入時自動調用 `__enter__`，離開區塊時自動調用 `__exit__`，而 `driver` 在 `main` 函數中的 `finally` 區塊手動進行關閉。
