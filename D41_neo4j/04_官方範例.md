...
6. 這裡可觀察 `WROTE` 關係節點中所顯示的結果，自己資料庫與官方資料庫似乎不同，下方是寫入自己資料庫後顯示的圖形。
...
10. 下載後，透過以下腳本可進行 JSON 文檔內容的比對。

    ```python
    import json

    def compare_json(file1, file2):
        """比較兩個JSON文件並打印不同之處，如果完全一致則輸出特定訊息"""
        with open(file1, 'r') as f1, open(file2, 'r') as f2:
            data1 = json.load(f1)
            data2 = json.load(f2)

        differences_found = False  # 跟踪是否發現差異
