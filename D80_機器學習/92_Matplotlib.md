...
3. 使顏色更加區別性，可以選擇一個具有更大色彩對比度的顏色映射方案（colormap），例如使用 plasma、coolwarm 或 inferno 等，也可以增加陰影或者邊界色，以突出圖形的細節；在機器學習模型中通常會有多個特徵，並希望了解這些特徵對目標變量的影響程度，通過這種 3D 圖可以將兩個主要特徵和模型輸出的結果進行可視化。
