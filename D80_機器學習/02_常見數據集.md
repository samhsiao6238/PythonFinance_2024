...
1. 與前面 `Wine Quality Dataset` 不同，另一個常見的數據集 `Wine Recognition Dataset` 也是與酒品有關的數據集，主要用於分類問題；這是來自 UCI 機器學習數據庫的分類數據集，用於根據化學分析來識別 3 種不同產地的意大利葡萄酒。數據集中的每一個數據點代表一個葡萄酒樣本，並包含 13 個與化學成分相關的特徵，例如酒精含量、蘋果酸、灰分、鎂含量等，包含 `178` 個樣本，每個樣本有 `13` 個特徵和一個目標變量。
...
2. 數據集包含 `150` 個樣本，每個樣本有 `4` 個特徵，目標變量（類別標籤）有 `3` 種不同的鳶尾花品種；適用於 `分類` 模型如 `k-NN`、`SVM`、`隨機森林`、`MLP` 等。
。
