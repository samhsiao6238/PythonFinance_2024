...
3. 以下是個進階範例，考慮多分類問題，並計算每個類別的預測概率，然後使用 `sklearn.metrics.log_loss` 來計算多類別的交叉熵損失，並可視化不同預測概率下的交叉熵損失。
    ...
    # 預測機率 y_pred_proba，這裡每個樣本的預測概率分佈在三個類別上
    y_pred_proba = np.array([
        [0.7, 0.2, 0.1],  # 樣本1的預測概率
        [0.1, 0.8, 0.1],  # 樣本2的預測概率
        [0.2, 0.2, 0.6],  # 樣本3的預測概率
        [0.1, 0.3, 0.6],  # 樣本4的預測概率
        [0.1, 0.7, 0.2],  # 樣本5的預測概率
        [0.8, 0.1, 0.1]   # 樣本6的預測概率
    ])
...
4. 輸出如下，`X 軸` 代表預測的類別 0 的概率（Predicted Probability for Class 0），`Y 縱` 代表交叉熵損失（Cross-Entropy Loss）；當預測概率接近 `0` 時，交叉熵損失接近 0，這表示模型的預測非常準確、損失很小；當預測概率逐漸偏向 `1` 時，交叉熵損失急劇增加，這是因為模型自信地預測錯誤，導致損失值變大。

    ![](images/img_54.png)

<br>

5. 交叉熵損失的特性是在真實標籤為 0 或 1 的情況下，損失的值取決於預測概率，當預測接近真實標籤時，損失值較低；當預測偏離真實標籤時，損失值增大；通過最小化交叉熵損失，模型調整權重，將使預測更接近真實標籤。
