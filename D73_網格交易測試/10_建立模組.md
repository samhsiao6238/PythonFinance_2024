...
2. 編輯內容。
    ...
    def check_and_match_columns(data_columns, required_columns):
        """
        確認數據列是否包含必要的列並進行匹配
        """
        # 將列名標準化為小寫
        ...
        for col in required_columns:
            ...
            if col_lower in data_columns_lower:
                original_name = data_columns[data_columns_lower.index(col_lower)]
                matched_columns[col] = original_name
            else:
                print(f"警告: 缺少列 '{col}'，將跳過該列處理。")
        return matched_columns

    def adjust_data_to_tick(data):
        """
        ...
        # 確認列名並進行匹配
        matched_columns = check_and_match_columns(data.columns, price_columns)

        # 遍歷匹配的列並進行調整
        for target_col, original_col in matched_columns.items():
            data[original_col] = data[original_col].apply(adjust_to_tick)
