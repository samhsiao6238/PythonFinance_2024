...
6. 查看欄位資訊，特別注意，結果顯示了 `復合欄位資訊（MultiIndex Columns）`，也就是欄位具備 `多重索引（MultiIndex）`，這類型的數據在欄位索引的每個層級中將提供多維度的訊息，以本例來說，第一個層次就是 `Price`，用於描述收盤價、開盤價等資訊，第二個層次描述的是股票代碼，例如這裡是 `0050.TW`。
