...
7. 查看欄位資訊，可看到結果顯示了 `復合欄位資訊（MultiIndex Columns）`，也就是欄位具備 `多重索引（MultiIndex）`，這類型的數據在欄位索引中會包含多維度的訊息；以本例來說，第一個層次就是 `Price`，用於描述收盤價、開盤價等資訊，第二個層次描述的是股票代碼，在這就是 `0050.TW`。
...
1. 處理數據，將 `MultiIndex` 欄位名稱簡化為 `單層索引`。
