...
1. 導入庫以及字型。
    ...
    rcParams['font.family'] = "STHeiti Medium.ttc"
...
3. 進行最佳化網格交易，測試多種參數組合下的交易策略，並計算其 `年化報酬率`，進而找到最優化的交易參數。
    ...
    for i, j, k in [
        ...
        print(f"網格間距：{i}，交易單位：{j}，上下差距比：{k}；年化報酬率：{yearly_return}")
...
2. 計算回測過程中的最大回撤（Maximum Drawdown, MDD），衡量策略的資金回撤風險。
...
3. 增加網格間距、交易單位和上下網格差距比的參數網格，並使用熱力圖（Heatmap）可視化性能。
...
4. 將市場數據劃分為 多頭、空頭和震盪市，並分別測試策略性能。
...
2. 繪製策略整個回測過程中的累計盈虧變化。
    ...
    plt.plot(position_table['進場時間'], position_table['累計盈虧'], label='累計盈虧', color='blue')
