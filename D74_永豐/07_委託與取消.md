...
2. 自訂義函數 `list_all_trades()` 遍歷委託明細，若有多筆委託則會逐一顯示；特別注意，這裡的命名與預設函數 `list_trades()` 類似但不同，封裝以利重用。

    ```python
    # 列出
    def list_all_trades():
        # 務必記得在函數內必須重新取得數據，不然每次調用都會使用舊數據
        today_trades = api.list_trades()
        # 遍歷
...
## 處理 PendingSubmit 的委託

_示範針對 `Pending` 狀態的委託單進行處理_

<br>

1. 調用自訂函數列出全部交易，確認當前委託單的狀態是 `PendingSubmit`。
...
2. 優化函數，查詢並取消所有 `PendingSubmit` 狀態的委託。

    ```python
    # 自訂函數：取消 Pending 中的委託
