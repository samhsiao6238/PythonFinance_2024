...
1. 初始化 Shioaji API，預設模式為 `True`，與官方 API 相同，代表 `模擬模式`。
...
3. 檢查是否傳入 API 物件；命名時使用前綴下底線，這在 Python 命名慣例中代表這是模組內部使用的私有函數。

    ```python
    # 取得已初始化的 Shioaji API 實例，若無則初始化
    def _get_or_init_api(api=None, simulation=True):
        # 如果傳入了 API，直接返回
        if api is not None:
            return api

        # 如果全域變數 `api` 存在且不為 None，返回全域變數
        if 'api' in globals() and globals()['api'] is not None:
            ...
            if current_api.simulation == simulation:
                ...
                print("API 模式不一致，重新初始化...")
            # 初始化新的 API 實例
            print("初始化新的 Shioaji API 實例...")
            api = init_Shioaji(simulation=simulation)
            # 若初始化失敗，函數會傳出 None，這裡將 None 寫入全局變數
            globals()['api'] = api
            return api

        # 初始化 API 並存入全域變數
        try:
            print("嘗試初始化 Shioaji API。")
            api = init_Shioaji()
            # 確保返回的 API 實例是唯一的，並保存在全域變數中
            globals()['api'] = api
            return api
        except Exception as e:
            print(f"Shioaji API 初始化失敗：{e}")
...
4. 啟用憑證，該函數會返回布林值，可供其他程序調用；補充説明，正式模式才需要啟用憑證，而啟用的前提是帳號必須已經登入，另外，憑證啟用時也會自動訂閱委託成交回報。
...
6. 檢查 API 登入狀態，這個函數將被其他函數調用，所以不輸出累贅資訊；如果已登入返回 True，否則返回 False；特別注意，這裡檢查的並非連線數，API 對於單一帳號的連線數有限制，但並無對應的函數可查詢連線數。
...
7. 嘗試並檢查是否登入 Shioaji 帳號，若未傳入 API，將進行初始化，若已登入則直接返回 API 物件；參數 `simulation` 預設值為 `True`，代表 `模擬模式`，若要正式下單則需顯式設置為 `False`，代表 `正式模式`；特別注意，帳號登入後無法切換模式，如需切換必須重新初始化 API 並再次登入。
    ...
    def login_Shioaji(
        ...
        try:
            ...
            try:
                ...
                if not simulation:
                    print("取得當前連線資訊...")
                    _monitor_connections(api, output_connections=True)
...
8. 登出全部 API 連線，依據參數 `reset_api` 決定是否在登出後重置 API，預設值為 `True`。

    ```python
    # 登出全部連線
    def logout_all_connections(
        api=None, reset_api=True
    ):
        # 檢查是否傳入 API
        api = _get_or_init_api()
    ...
    def _monitor_connections(api, output_connections=True):
        try:
            if output_connections:
                status = api.usage()
                if not status:
                    print("模擬環境可能不支援 usage() 或尚未登入，無法取得連線資訊。")
                    return None
                connections = status.connections
                print(f"當前連線數：{connections}")
                print(f"當前流量：{status.bytes / (1024 * 1024):.2f} MB")
                print(f"每日流量限制：{status.limit_bytes / (1024 * 1024):.2f} MB")
                print(f"剩餘流量：{status.remaining_bytes / (1024 * 1024):.2f} MB")
                return connections
        except Exception as e:
            print(f"無法檢查連線資訊（可能是模擬環境不支援或尚未登入）：{e}")
            return None
...
12. 重新載入 `MyShioaji` 模組，預設使用初次導入時的別名 `msj`；因為後續步驟會有多次編輯模組的需求，避免每次重啟核心造成多次連線，所以封裝重載模組的函數。

    ```python
    def reload_myshioaji_module(module_alias: str = "msj"):
        import sys
        import importlib
...
_完成登入函數之後，可測試啟用憑證，因為這個函數會在沒有 api 時調用登入函數；根據 Shioaji 官方文件，在 `正式模式` 下才有啟用憑證的需求，這是用來確保交易的安全性及身份驗證；在 `模擬模式` 中是不需要啟用憑證的。_

<br>

1. 調用自訂函數啟用憑證；特別注意，假如尚未初始化 API，這時會調用初始化函數，並將 API 對象寫入全局變數 `api` 中。
...
_`【重啟核心】` 示範切換或手動輸入密鑰，點擊筆記本上方的 `Clear All Outputs`，然後點擊 `Restart`。_
...
2. 登出所有連線但保留原始 API，也就是不刪除線有設定與資訊，僅執行登出；當有記憶體泄漏疑慮或釋放資源需求時，建議重置 API。

    ```python
    # 登出所有連線但保留原始 API
    msj.logout_all_connections(api, reset_api=False)
    ```
...
_`【重啟核心】` 進行測試是否從伺服器下載 `商品檔`，也就是 `合約資料`。_
...
_`【重啟核心】` 是否訂閱委託成交回報，維持預設值 `True`，並未寫入 `login_Shioaji`，以下僅作說明。_
