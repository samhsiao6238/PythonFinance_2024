...
4. 嘗試並檢查是否登入 Shioaji 帳號，若未傳入 API，將進行初始化，若已登入則直接返回 API 物件；參數 `simulation` 預設值為 `True`，代表 `模擬模式`，若要正式下單則需顯式設置為 `False`，代表 `正式模式`，登入成功後應自動呼叫 `activate_ca(api)` 來啟用憑證；特別注意，帳號登入後無法切換模式，如需切換必須重新初始化 API 並再次登入。
