...
3. 初始化 Shioaji API，預設模式為 `True`，與官方 API 相同，代表 `模擬模式`。
...
5. 檢查是否傳入 API 物件；命名時使用前綴下底線，這在 Python 命名慣例中代表這是模組內部使用的私有函數。
...
9. 登出全部 API 連線，依據參數 `reset_api` 決定是否在登出後重置 API，預設值為 `True`；如果尚未登入，直接回傳原始 API 實例；若已登入，顯示當前連線數，並調用 `logout()`。
...
9. 登出全部 API 連線，依據參數 `reset_api` 決定是否在登出後重置 API，預設值為 `True`；如果尚未登入，直接回傳原始 API 實例；若已登入，顯示當前連線數，並調用 `logout()`。
    ...
    ```python
    def logout_all_connections_v2(api=None, reset_api=True):
        ...
        try:
            ...
            if not check_login_status(api):
                print("未偵測到登入帳戶，無需登出。")
                return api  # 返回原始 API 以繼續操作

            # === Step 1: 顯示當前連線數 ===
            try:
                # 假設能透過 api.list_connections() 或類似方法取得連線資訊
                connections_before = len(api.list_connections())
                print(f"【登出前】連線數：{connections_before}")
            except Exception as e:
                print(f"【登出前】無法取得連線數，可能原因：{e}")
            ...
            try:
                ...
                # logout() 一般會回傳 True 表示成功
                if logout_result is True:
                    print("登出成功，所有連線已釋放。")
                else:
                    print(f"登出結果：{logout_result}（預期為 True）")
            except Exception as e:
                print(f"登出失敗：{e}")
                # 如果登出失敗，返回原始 API 實例
                return api
            ...
            # === Step 3: 再次測試登出後的連線數 ===
            try:
                connections_after = len(api.list_connections())
                ...
                # 若呼叫 list_connections() 出現錯誤，即表示無法再存取連線資訊
                print(f"【登出後】無法取得連線數（預期可能登出成功而取不到）：{e}")
