...
_暫時禁用或重定向標準輸出可避免執行某些操作時，底層函數如 `api.place_order()` 直接將輸出打印到控制台，導致不必要的輸出干擾主程式的執行流程。_
...
2. 將標準輸出重定向到 `io.StringIO()`，這是一個內存中的文件對象，可以捕獲所有被寫入的輸出，任何輸出到 `sys.stdout` 的內容，現在都會被寫入到這個內存對象中，而不再顯示在控制台上，如此便可阻止如 `api.place_order()` 函數直接輸出的內容顯示在控制台中，暫時屏蔽底層方法的輸出，避免影響主程式的控制台輸出。
