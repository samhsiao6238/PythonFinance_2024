...
_以下同樣要重啟核心進行測試是否從伺服器下載 `商品檔`，也就是 `合約資料`。_
...
2. 是否訂閱 `委託/成交回報`，預設為 `True`，這裡設置為 `False` 進行測試。
...
6. 特別注意，`Shioaji` 沒有直接的方法來檢查是否訂閱了成交回報，但可透過設置 _回調函數_ 來監控成交回報，如果有回報進來，代表訂閱成功；關於 _回調函數_ 會另作完整說明。
