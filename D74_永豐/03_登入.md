...
8. 登出全部 API 連線，依據參數 `reset_api` 決定是否在登出後重置 API，預設值為 `True`。
    ...
    ```python
    # 登出全部連線
    def logout_all_connections(api=None, reset_api=True):
        # 檢查是否傳入 API
        api = _get_or_init_api()
...
10. 若在登入時未訂閱委託成交回報，可在需要時手動訂閱。
    ...
    ```python
    def manual_subscribe_trade(api=None):
        # 檢查是否傳入 API
        api = _get_or_init_api()
...
_說明兩種不同載入環境變數方式的差異，分別是 `os.environ["<變數名稱>"]` 和 `os.environ.get("<變數名稱>")`。_
...
1. 調用自訂函數啟用憑證；特別注意，以下函數必須在 API 對象建立之後調用。
...
_示範切換或手動輸入密鑰，需先重啟核心，點擊筆記本上方的 `Clear All Outputs`，然後點擊 `Restart`。_
...
_同樣再次重啟核心，點擊筆記本上方的 `Clear All Outputs`，然後點擊 `Restart`。_
...
_是否從伺服器下載 `商品檔`，也就是 `合約資料`；維持預設值 `True`，並未寫入 `login_Shioaji`，以下僅作說明。_
...
2. 是否從伺服器下載商品檔 `fetch_contract`，預設為 `True`；若在需要快速登入的情境中可設置為 `False`，此時調用與合約相關的 API 會報錯。
...
4. 在登入時若未下載合約資料，可在需要時以手動方式下載所需的合約資料。
...
_是否訂閱委託成交回報，維持預設值 `True`，並未寫入 `login_Shioaji`，以下僅作說明。_
...
3. 特別注意，`Shioaji` 沒有直接的方法來檢查是否訂閱了成交回報，但可透過設置 _回調函數_ 來監控成交回報，如果有回報進來，代表訂閱成功；關於 _回調函數_ 會另作完整說明。
