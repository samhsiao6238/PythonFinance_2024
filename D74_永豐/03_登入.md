...
3. 檢查是否傳入 API 物件；命名時使用前綴下底線，這在 Python 命名慣例中代表這是模組內部使用的私有函數。
    ...
    def _get_or_init_api(api=None):
        ...
        if api is not None:
            # 直接返回
            return api
        # 若未傳入 api，檢查全域是否存在變數 api
        if 'api' in globals() and globals()['api'] is not None:
            api = globals()['api']
            return api
        # 全域無 api 變數或為 None，嘗試初始化
        try:
            print("嘗試進行 Shioaji API 初始化。")
            # 呼叫初始化函數
            api = init_Shioaji()
            if not api:
                print("API 初始化失敗，無法繼續操作。")
                return None
            # 將成功的 API 存入全域變數
            globals()['api'] = api
            return api
...
_繼續編輯模組 `MyShioaji.py`；特別注意，這裡檢查的並非連線數，API 對於單一帳號的連線數有限制，但並無對應的函數可查詢連線數。_

<br>

1. 檢查 API 登入狀態，這個函數將被其他函數調用，所以不輸出累贅資訊；如果已登入返回 True，否則返回 False。
...
3. 再次調用登入函數，因為環境變數已經載入，所以登入時可將其傳入。
...
4. 同樣再次重啟核心；調用函數初始化 API 對象，並設定為模擬模式。

    ```python
    # 初始化 API 對象
    api = msj.init_Shioaji(simulation=True)
    ```
...
5. 調用登入函數，傳入 API 對象，這時雖然會傳出 API 對象，但傳出的會是原本的對象。
...
2. 基於這個基礎，在模組中建立函數檢查當前全部的登入帳號是否都完成簽署。
...
3. 檢查當前 API 對象所執行的全部登入帳號的簽署狀態。

    ```python
    # 檢查當前全部登入帳號的簽署狀態
    msj.check_unsigned_accounts(api)
    ```
...
3. 登出所有連線但保留原始 API，也就是不刪除線有設定與資訊，僅執行登出；當有記憶體泄漏疑慮或釋放資源需求時，建議重置 API。
