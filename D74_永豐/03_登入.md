...
1. 登出全部 API 連線，依據參數 `reset_api` 決定是否在登出後重置 API，預設值為 `True`。
    ...
    def logout_all_connections(api, reset_api=True):
        """
        登出所有 Shioaji API 連線，並根據參數選擇是否重置 API 實例。

        Args:
            api (Shioaji): 已初始化的 Shioaji API 實例。
            reset_api (bool): 是否在登出後重置 API 實例。預設為 True。

        Returns:
            Shioaji: 如果重置，返回新的 API 實例；如果不重置，返回原始 API 實例。
        """
        try:
            # 檢查是否已登入
            if not check_login_status(api):
                print("未偵測到登入帳戶，無需登出。")
                return api  # 返回原始 API 實例以繼續操作
            ...
            try:
                ...
                print(f"登出失敗：{e}")
                return api  # 如果登出失敗，返回原始 API 實例

            # 根據參數選擇是否重置 API 實例
            if reset_api:
                print("正在重置 API 實例...")
                api = sj.Shioaji()  # 重新初始化 API
...
2. 登出所有連線但保留原始 API，也就是不刪除線有設定與資訊，僅執行登出；當有內存泄漏疑慮或釋放資源需求時，建議重置 API。
    ...
    ```python
    # 登出所有連線但保留原始 API
    api = logout_all_connections(api, reset_api=False)
    ```
