...
1. 先查詢確認委託，接著取消全部有效的委託單。
    ...
    ```python
    # 取消所有有效的委託單
    def cancel_all_valid_orders(api):
        try:
            # 取得有效委託單
...
3. 特別注意，API 所傳出的 `response` 並非標準字典，若進行解析將會出錯；以下嘗試使用正則表達式提取 OrderState.StockOrder 的 JSON 格式內容，接著使用 json.loads 轉換為 Python 字典進行解析。

    ```python
    # 使用正則表達式解析非標準格式的 response
...
4. 優化取消有效訂單的函數。
    ...
    ```python
    # 基於正則處理的回應取消全部有效訂單
    def cancel_all_valid_orders_with_regex(api):
        try:
            # 取得有效委託單
            valid_trades = list_current_orders(api)
            if not valid_trades:
...
5. 調用函數取消全部有效訂單。
    ...
    ```python
    msj.cancel_all_valid_orders_with_regex(api)
    ```
