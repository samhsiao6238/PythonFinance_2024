...
1. 程式碼。
    ...
    async def on_message(message):
        ...
        if discord_client.user.mentioned_in(message):
            ...
            # 檢查 prompt 是否為空，或僅包含空白字符；特別注意， not 僅使用在前面的表達式中
            ...
            if message.reference:
                print("訊息是另一則訊息的回覆")
                # 以下這段代碼嘗試從訊息所屬的頻道中獲取當前訊息所回覆的那條原始訊息
                # 獲取成功會將訊息儲存在變數 replied_message 中
                # 如果獲取失敗，則會進入 except 處理異常情況
                try:
                    # 嘗試從訊息所屬的頻道中獲取被回覆的原始訊息
